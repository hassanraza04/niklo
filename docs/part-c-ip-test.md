...
- gosom: the docker image is **amd64-only**, and its bundled chromium segfaults
  ...
  **native arm64 binary** (`~/go/bin/google-maps-scraper`, v1.14), which is native
  chromium with no emulation. That's what `run.sh` calls.
...
Decided not to bother with the CI cells (A/B), since the whole point is the local IP,
...
synonyms plus the DHA/Clifton neighbourhood passes that pulled the rest in. And the
...
- The broad set includes big multi-sport clubs (Gymkhana, DA Club, Creek Club).
  Most of those genuinely added padel courts, so they stay, and curation trims the
  rest later.
